...
var _road_toolbar:RoadToolbar
var _eds = EditorInterface.get_selection()
...
func _enter_tree():
	...
	#road_point_editor.call("set_edi", EditorInterface)
...
func select_road_point(point) -> void:
	...
	EditorInterface.get_selection().add_node(point)
	point.on_transform()
	_show_road_toolbar()
...
func set_selection(node: Node) -> void:
	EditorInterface.get_selection().clear()
	# EditorInterface.edit_node(node) # Necessary?
