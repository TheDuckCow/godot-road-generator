...
func handle_shortcuts(event: InputEvent, camera: Camera3D) -> void:
	...
	var _did_select := handle_selection_leftclick(event, camera)


## Returns true if this did process as a selection event
##
## Used to trigger selection of the nearest RoadPoint when you click onto a mesh
## generated by a road node, to jump to the nearest control node.
##
## TODO: get this to somehow run even a RoadSegment is the only mesh in front of
## the 3D void, as no gui input is triggered then.
func handle_selection_leftclick(event: InputEvent, camera: Camera3D) -> bool:
	...
	if point and not event.pressed:
		# TODO: See if this needs to update.
		# Previously, used the deferred assignment:
		# plg._new_selection = point.container
		# As using set_selection(point) directly created a conflcit with builtin
		# drag n drop & 3d gizmo usage
		if point.container.is_subscene():
			plg.set_selection.call_deferred(point.container)
		else:
			plg.set_selection.call_deferred(point)
		return true
	return false
