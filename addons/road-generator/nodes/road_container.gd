...
@icon("../resources/road_container.png")
...
@export var edge_containers:Array[NodePath] # (Array, NodePath)
...
@export var edge_rp_target_dirs:PackedInt32Array # (Array, int)
# Node paths within this container, relative to this container
@export var edge_rp_locals:Array[NodePath] # (Array, NodePath)
# Local RP directions, enum value of RoadPoint.PointInit
@export var edge_rp_local_dirs:PackedInt32Array  # # (Array, int)
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if get_tree().get_edited_scene_root() != self:
		...
		while true:
			if _last_par.get_path() == NodePath("/root"):
				break
...
func validate_edges(autofix: bool = false) -> bool:
	...
	for _idx in range(len(edge_rp_locals)):
		...
		if edge_containers[_idx].is_empty() == false:
...
func segment_rebuild(road_segment:RoadSegment):
	road_segment.check_rebuild_road()
