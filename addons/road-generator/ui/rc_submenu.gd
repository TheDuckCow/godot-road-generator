...
func _ready() -> void:
	...
	dir_selector.mode = FileDialog.MODE_OPEN_DIR
	dir_selector.mode_overrides_title = true
	dir_selector.add_filter("*.tscn ; TSCN files")
	dir_selector.window_title = "Select folder with RoadContainer TSCN files"
	var res = dir_selector.connect("dir_selected", self, "_on_folder_selected")
	assert(res == OK)
	add_child(dir_selector)


func _enter_tree() -> void:
	connect("id_pressed", self, "_create_menu_item_clicked")


func _exit_tree() -> void:
	disconnect("id_pressed", self, "_create_menu_item_clicked")
