...
func _notification(what):
	...
	if what == NOTIFICATION_TRANSFORM_CHANGED:
		...
		on_transform_road(low_poly)


func on_transform_road(low_poly=false):
	...
	print_debug("TODO: implement gizmo check")
	#if is_instance_valid(get_gizmos()[0]):
...
func connect_container(this_direction: int, target_rp: Node, target_direction: int) -> bool:
	...
	# container.update_edges()
	on_transform_road() # Only changes that should happen: Update connections of AI lanes.
	target_rp.on_transform_road()
	return true
...
func validate_junctions():
	...
	var _tmp_ref
	if prior_pt_init and not prior_pt_init.is_empty():
		_tmp_ref = get_node(prior_pt_init)
		if _tmp_ref.has_method("is_road_point"):
			prior_point = _tmp_ref
	if next_pt_init and not next_pt_init.is_empty():
...
func _is_junction_valid(point: RoadPoint)->bool:
	...
	if point.prior_pt_init and not point.prior_pt_init.is_empty():
		prior_point = get_node(point.prior_pt_init)
	if point.next_pt_init and not point.next_pt_init.is_empty():
...
func _autofix_noncyclic_references(
	...
	elif for_prior and point.next_pt_init.is_empty():
